...
type Persisted = { ts: number; state: ReturnType<typeof dehydrate> };
...
export function restoreQueryCache(client: QueryClient): void {
  try {
    ...
    if (!parsed?.state || Date.now() - parsed.ts > MAX_AGE_MS) {
      localStorage.removeItem(KEY);
      return;
    }
...
export function startQueryPersist(client: QueryClient): () => void {
  ...
  const flush = () => {
    ...
    try {
      ...
      const payload = JSON.stringify({ ts: Date.now(), state } satisfies Persisted);
