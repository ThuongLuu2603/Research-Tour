...
export default function IntelligenceHome() {
  ...
  return (
    <div className="p-6 space-y-6 w-full">
      ...
      {/* Lễ hội sắp tới — VTR đang thiếu độ phủ (Wave 6: festival thành dimension trên Home) */}
      {(data?.festivals?.top_gaps?.length ?? 0) > 0 && (
        <div className="card p-5">
          <div className="flex items-center justify-between mb-1">
            <h2 className="font-semibold flex items-center gap-2">
              🎉 Lễ hội sắp tới — cơ hội phủ tour
            </h2>
            <Link to="/festivals" className="text-xs text-primary-600">Mở Sự kiện & Lễ hội →</Link>
          </div>
          <p className="text-xs text-gray-500 mb-3">
            {data?.festivals?.upcoming_count ?? 0} lễ sắp diễn ra · {data?.festivals?.gap_count ?? 0} lễ đối thủ phủ nhiều hơn VTR. Ưu tiên mở tour theo các lễ dưới đây.
          </p>
          <div className="space-y-2">
            {(data?.festivals?.top_gaps ?? []).map((f) => (
              <Link
                key={f.slug}
                to={`/festivals?focus=${encodeURIComponent(f.slug)}`}
                className="flex items-center justify-between gap-3 rounded-lg border border-transparent hover:border-amber-300 hover:bg-amber-50 px-3 py-2 transition-colors"
              >
                <div className="min-w-0">
                  <div className="text-sm font-medium truncate">{f.name}</div>
                  <div className="text-xs text-gray-500">
                    {f.date_start?.slice(5)}–{f.date_end?.slice(5)} · {f.region}
                  </div>
                </div>
                <div className="flex items-center gap-3 shrink-0 text-xs">
                  <span className="text-gray-500">VTR <b className="text-primary-700">{f.vtr_tours}</b></span>
                  <span className="text-gray-500">ĐT <b className="text-red-600">{f.competitor_tours}</b></span>
